...
switch_status_t rtsp_new_channel(switch_core_session_t **new_session, int remote_video_rtp_port, const char *destination_number) {
	...
	const char *err;
	const char *ip;
	switch_rtp_flag_t flags[SWITCH_RTP_FLAG_INVALID] = {0};//by aphero
	...
	tech_pvt->rtp_session = switch_rtp_new(ip, //local
		...
		1,
		8000,
		flags,//SWITCH_RTP_FLAG_AUTOADJ | SWITCH_RTP_FLAG_DATAWAIT | SWITCH_RTP_FLAG_VIDEO,
	...
	switch_rtp_activate_rtcp(tech_pvt->rtp_session, 1000, 4441,SWITCH_FALSE);
	switch_rtp_activate_rtcp(tech_pvt->video_rtp_session, 1000, remote_video_rtp_port + 1,SWITCH_FALSE);
...
static switch_status_t rtsp_receive_message(switch_core_session_t *session, switch_core_session_message_t *msg)
{
	...
	case SWITCH_MESSAGE_INDICATE_JITTER_BUFFER:
		break;
	case SWITCH_MESSAGE_INDICATE_DEBUG_MEDIA://SWITCH_MESSAGE_INDICATE_DEBUG_AUDIO: //by aphero
		{
	...
	while(switch_channel_ready(channel) && helper->running) {
...

		{
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
				"%x %x | ft: %d, nt: %d, sb: %d len: %d m: %d ts: %d\n",
...
user="admin";
pass="12345";
switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),SWITCH_LOG_DEBUG, "%s %d %s %s\n", host,port,user,pass);
//2018-01-14 17:38:39.910263 [DEBUG] mod_rtsp.c:1546 admin:12345@110.167.199.206 554 (null) (null)
	...
	while(switch_socket_recv(new_sock, p, &len) == SWITCH_STATUS_SUCCESS) {
		...
		p[len] = '\0';
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "read bytes:%" SWITCH_SIZE_T_FMT "\n", len);
	...
	flags[SWITCH_RTP_FLAG_AUTOADJ]++;
						...
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "body:%s\n", rtsp_msg->body);
					}
					break;
				default:
					msg = switch_mprintf("RTSP/1.0 405 Method Not Allowed\r\n"
